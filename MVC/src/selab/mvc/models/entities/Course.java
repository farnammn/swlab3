...
public class Course implements Model {
    ...
    private DataSet<StudentCourse> students = new DataSet<>();
    ...
    public void addStudent(StudentCourse sc){
        students.add(sc);
    }

    public void removeStudent(String studentNo){
        if(students.containsKey(this.courseNo + " - " + studentNo)) {
            StudentCourse sc = students.get(this.courseNo + " - " + studentNo);
            students.remove(sc);
        }
    }
    ...
    public float getAverage() {
        ...
        int sum = 0;
        int i = 0;
        for (StudentCourse s: students.getAll()){
            sum += Integer.parseInt(s.getPoints());
